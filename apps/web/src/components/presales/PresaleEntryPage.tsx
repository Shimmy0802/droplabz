...
interface PresaleEntryPageProps {
    eventId: string;
}
...
export function PresaleEntryPage({ eventId }: PresaleEntryPageProps) {
    ...
    useEffect(() => {
        const fetchEventDetails = async () => {
            try {
                ...
                console.error('Error loading presale:', err);
    ...
    if (loading) {
        return <div className="text-center py-12 text-white">Loading presale details...</div>;
    }

    if (!eventDetails) {
        return (
            <div className="text-center py-12">
                <p className="text-red-400">Presale not found</p>
            </div>
        );
    }
    ...
    if (eventDetails.status !== 'ACTIVE') {
        return (
            <div className="text-center py-12">
                <p className="text-yellow-400">This presale is no longer accepting applications</p>
            </div>
        );
    }
        ...
        <div className="min-h-screen text-white p-6">
            <div className="max-w-2xl mx-auto">
                ...
                <div className="mb-8">
                    ...
                    <h2 className="text-2xl font-bold text-[#00d4ff]">{eventDetails.community.name}</h2>
                </div>
                ...
                <div className="bg-[#111528] border border-[#00d4ff]/20 rounded-lg p-6 mb-8">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <p className="text-sm text-gray-400">Allocation Spots Available</p>
                ...
                {eventDetails.requirements.length > 0 && (
                    <div className="bg-[#111528] border border-[#00d4ff]/20 rounded-lg p-6 mb-8">
                        <h3 className="text-lg font-semibold text-[#00d4ff] mb-4">Eligibility Requirements</h3>
                ...
                <form onSubmit={handleSubmit} className="bg-[#111528] border border-[#00d4ff]/20 rounded-lg p-6">
                    <h3 className="text-lg font-semibold text-white mb-6">Apply to Presale</h3>
                    ...
                    <div className="mb-6">
                        ...
                        <p className="text-xs text-gray-400 mt-1">
                            Your Discord will be verified against the presale requirements
                        </p>
                    </div>
                    ...
                    {success && (
                        <div className="mb-6 p-4 bg-green-900/20 border border-green-500 rounded-lg text-green-200">
                            ✓ Application submitted! Redirecting to community page...
                        </div>
                    )}
                    ...
                    <button
                        ...
                        className="w-full px-6 py-3 bg-[#00d4ff] text-[#0a0e27] font-semibold rounded-lg hover:bg-[#0099cc] disabled:opacity-50 disabled:cursor-not-allowed transition"
                    >
                        {submitting ? 'Submitting...' : 'Apply to Presale'}
                    </button>

                    <p className="text-xs text-gray-400 mt-4 text-center">
                        Your Solana wallet and Discord account will be verified. Selected applicants will receive
                        allocation details.
                    </p>
                </form>
            </div>
